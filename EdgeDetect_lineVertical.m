...
%Edge Detection Filter: line edge detect (Vertical) filter
%parameter: -original image

function result=EdgeDetect_lineVertical(in)
in = double(in);
[r,c,ch]=size(in);
new=ones(r,c,ch);
...
for i=2:r+1
        for j=2:c+1
            ...
            res(i-1,j-1)=( (out(i-1,j+1)) + (-1*out(i+1,j+1)) ...
            + (2*out(i-1,j)) + (-2*out(i+1,j)) ...
            + (1*out(i-1,j-1)) + (-1*out(i+1,j-1)) );
        end
end
