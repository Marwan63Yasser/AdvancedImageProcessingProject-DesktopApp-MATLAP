...
%Restoration: Add (Salt&pepper) Noise THEN Remove noise using Mid-point (order filter)
%Parameters:-Original image  -Mask side size like 3 for 3*3 mask,
%                             5 for 5*5 mask and so on, (optional - default:3)

function result=saltPepper_midPoint(in,mask)
...
if (ch == 1)
    for i=1+ext:r+ext
        for j=1+ext:c+ext
            ...
            res(i-ext,j-ext) = (max(arr)+min(arr))/2;
            p = 1;
        end
    end
    ...
    for i=1+ext:r+ext
      for j=1+ext:c+ext
           ...
           for x = inv_ext:ext
            ...
            %choose the maximum element and put it in the pixel
            res(i-ext,j-ext,1) = (max(arr)+min(arr))/2;
            p = 1;
           ...
           for x = inv_ext:ext
            ...
            %choose the maximum element and put it in the pixel
            res(i-ext,j-ext,2) = (max(arr)+min(arr))/2;
            p = 1;
           ...
           for x = inv_ext:ext
            ...
            %choose the maximum element and put it in the pixel
            res(i-ext,j-ext,3) = (max(arr)+min(arr))/2;
            p = 1;
