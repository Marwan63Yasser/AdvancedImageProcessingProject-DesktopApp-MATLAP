...
%Sharpening Filter: Line Sharpening filter (Vertical)
%parameter: -original image

function result=Sharp_lineVertical(in)
in = double(in);
[r,c,ch]=size(in);
res=ones(r,c,ch);
...
if (ch == 1)
    for i=2:r+1
            for j=2:c+1
                ...
                res(i-1,j-1) = (out(i,j+1)) + (out(i,j)) + (-1*out(i,j-1));
            end
    end
    ...
    for i=2:r+1
            for j=2:c+1
                ...
                res(i-1,j-1,1) = (out(i,j+1,1)) + (out(i,j,1)) + (-1*out(i,j-1,1));

                %apply the mask on each pixel in the image - (GREEN Component)
                res(i-1,j-1,2) = (out(i,j+1,2)) + (out(i,j,2)) + (-1*out(i,j-1,2));

                %apply the mask on each pixel in the image - (BLUE Component)
                res(i-1,j-1,3) = (out(i,j+1,3)) + (out(i,j,3)) + (-1*out(i,j-1,3));
            end
    end
