...
%Edge Detection Filter: line edge detect (Horizontal) filter
%parameter: -original image

function result=EdgeDetect_lineHorizontal(in)
in = double(in);
[r,c,ch]=size(in);
new=ones(r,c,ch);
...
for i=2:r+1
        for j=2:c+1
            ...
            res(i-1,j-1)=( (out(i-1,j+1)) + (2*out(i,j+1)) + (out(i+1,j+1)) ...
            + (-1*out(i-1,j-1)) + (-2*out(i,j-1)) + (-1*out(i+1,j-1)) );
        end
end
